...
public class TreeNode<T> implements Iterable<T>, Cloneable, Serializable
{
    ...
    private static <E> Iterator<TreeNode<E>> emptyIterator() {
        return Collections.<TreeNode<E>>emptyList().iterator();
    }
    ...
    public TreeNode(T userObject, boolean allowsChildren) {
        super();
        parent = null;
     ...
     */
    public boolean isNodeDescendant(TreeNode<T> anotherNode) {
        if (anotherNode == null)
            return false;
     ...
     *          or null if none
     */
    public TreeNode<T> getSharedAncestor(TreeNode<T> aNode) {
        if (aNode == this) {
            return this;
        } else if (aNode == null) {
            return null;
     ...
     */
    public boolean isNodeRelated(TreeNode<T> aNode) {
    ...
    public int getDepth() {
        ...
        Iterator<TreeNode<T>> enum_ = breadthFirstEnumeration();
    ...
    public TreeNode<T> getNextNode() {
        if (getChildCount() == 0) {
            ...
            TreeNode<T> nextSibling = getNextSibling();

            if (nextSibling == null) {
                TreeNode<T> aNode = getParent();
     ...
     */
    public Iterator<TreeNode<T>> preorderEnumeration() {
     ...
     */
    public Iterator<TreeNode<T>> postorderEnumeration() {
     ...
     */
    public Iterator<TreeNode<T>> breadthFirstEnumeration() {
     ...
     */
    public Iterator<TreeNode<T>> depthFirstEnumeration() {
     ...
     */
    public Iterator<TreeNode<T>> pathFromAncestorEnumeration(TreeNode<T> ancestor) {
    ...
    public int getLeafCount() {
        ...
        TreeNode node;
        Iterator<TreeNode<T>> enum_ = breadthFirstEnumeration(); // order matters not
    ...
    public String toString() {
        ...
        return "TreeNode{hasParent=" + !isRoot() + ",children=" + getChildCount() + ",label=" + abbreviatedLabel + "}";
    }
    ...
    public TreeNode<T> clone() {
        ...
        try {
            ...
            newNode = (TreeNode<T>) super.clone();
    ...
    private final static class PreorderEnumeration<E> implements Iterator<TreeNode<E>> {
        ...
        public TreeNode<E> next() {
            Iterator<TreeNode<E>> enumer = stack.peek();
            TreeNode<E> node = enumer.next();
            Iterator<TreeNode<E>> children = node.children();
    ...
    private final static class PostorderEnumeration<E> implements Iterator<TreeNode<E>> {
        protected TreeNode<E> root;
        protected Iterator<TreeNode<E>> children;
        protected Iterator<TreeNode<E>> subtree;

        public PostorderEnumeration(TreeNode<E> rootNode) {
            super();
            root = rootNode;
    ...
    private static final class BreadthFirstEnumeration<E> implements Iterator<TreeNode<E>> {
        ...
        public TreeNode<E> next() {
            Iterator<TreeNode<E>> enumer = queue.firstObject();
            TreeNode<E> node = enumer.next();
            Iterator<TreeNode<E>> children = node.children();
    ...
    private static final class PathBetweenNodesEnumeration<E> implements Iterator<TreeNode<E>> {
        protected Stack<TreeNode<E>> stack;

        public PathBetweenNodesEnumeration(TreeNode<E> ancestor,
                                           TreeNode<E> descendant)
        {
            super();
    ...
    public static <E> Function<TreeNode<E>, String> infoFunction() {
        return input -> "TreeNode{isRoot=" + input.isRoot()
    ...
    public static <E> Function<TreeNode<E>, E> userObjectFunction() {
    ...
    public Iterator<T> iterator() {
        return stream().map(TreeNode::getUserObject).iterator();
    }

    public Stream<TreeNode<T>> stream() {
        Iterator<TreeNode<T>> it = preorderEnumeration();
        Iterable<TreeNode<T>> iterable = this::preorderEnumeration;
        Stream<TreeNode<T>> stream = StreamSupport.stream(iterable.spliterator(), false);
        return stream;
    }
